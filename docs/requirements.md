...
印刷・PDF・校正プリフライトスイート は、印刷物やPDF公開物は確認点がアプリごとに分かれ、入稿直前に不備が見つかりやすい。 という課題を、Acrobat JavaScript + external evidence CLI で解決する。
...
- pdf preflight case を JSON 入力で受け取り、必須項目を検証できる。
