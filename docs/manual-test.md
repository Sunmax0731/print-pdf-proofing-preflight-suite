...
- 作業ディレクトリ: `D:\AI\AdobePlugin\Acrobat\print-pdf-proofing-preflight-suite`
...
```powershell
cd D:\AI\AdobePlugin\Acrobat\print-pdf-proofing-preflight-suite
npm test
npm start
```
...
5. Acrobat JavaScript + external evidence CLI の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/print-pdf-proofing-preflight-suite-docs.zip` に存在する。
